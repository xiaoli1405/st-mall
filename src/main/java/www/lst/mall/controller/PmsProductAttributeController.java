...
public class PmsProductAttributeController {
    ...
    @ApiOperation("分页获取商品属性管理")
    @ApiImplicitParams({@ApiImplicitParam(name = "type", value = "0表示属性，1表示参数", required = true, paramType = "query", dataType = "integer")})
    @RequestMapping(value = "/getList/{cid}", method = RequestMethod.GET)
    @ResponseBody
    ...
    public CommonResult create(@RequestBody PmsProductCategoryParam productCategoryParam) {
        int count = productAttributeService.create(productCategoryParam);
        if (count > 0) {
    ...
    public CommonResult update(@PathVariable Long id,
        ...
        int count = productAttributeService.update(id, productCategoryParam);
        if (count > 0) {
            return CommonResult.success(count);
        }else {
            return CommonResult.failed();
    ...
    @ApiOperation("根据商品分类的id获取商品属性及分类")
    @RequestMapping(value = "/attrInfo/{productCategoryId}", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<ProductAttrInfo>> getAttrInfo(Long productCategoryId) {
